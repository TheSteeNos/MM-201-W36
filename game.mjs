...
CONTINUE_PROMPT: "Want to play again? Enter 1 for yes, or 2 for no: ",
THANK_PLAYER: "Thank you for playing!",
STAT_INFO: "Statistics: ",
WRONG_GUESS_AMOUNT: "Number of wrong guesses: ",
PLAYER_ACCEPT: "1",
PLAYER_DENY: "2",
};

const CHAR_AND_WORDLIST = {
SPACE: " ",
EMPTY: "",
UNDERBRACKET: "_",
}
...
while (willPlayerContinue == true) {
    ...
    let wordDisplay = CHAR_AND_WORDLIST.EMPTY;
    let correctWord = words[getRandomNumber(words.length)];
    const numberOfCharInWord = correctWord.length;
    let guessedWord = CHAR_AND_WORDLIST.EMPTY.padStart(correctWord.length, CHAR_AND_WORDLIST.UNDERBRACKET);
    ...
    while (isGameOver == false) {
        ...
        if (answer == correctWord) {
            ...
            let org = guessedWord;
            guessedWord = CHAR_AND_WORDLIST.EMPTY;
    ...
    if (wasGuessCorrect) {
        console.log(ANSI.COLOR.YELLOW + PLAYER_PROMPTS_AND_INFO.WIN);
    }
    ...
    totalWrongGuesses.push(wrongGuesses);
    wrongGuesses = [];

    const answer = (await askQuestion(PLAYER_PROMPTS_AND_INFO.CONTINUE_PROMPT));
...
console.log(PLAYER_PROMPTS_AND_INFO.WRONG_GUESS_AMOUNT + ANSI.COLOR.RED + totalWrongGuesses.length + ANSI.RESET);
